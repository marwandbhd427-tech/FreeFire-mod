//
//  FREE FIRE iOS VIP MOD MENU v4.2 – FINAL FIX
//
...
// ═══════════════════════════════════════════════════════════════
// DOBBY DECLARATIONS (بدل include لمنع خطأ extern "C")
// ═══════════════════════════════════════════════════════════════
extern "C" {
...
void RunAimbotAndESP(ImDrawList* draw_list, ImVec2 screenSize) {
    ...
    if ((cfg.Aimbot || cfg.Aimbot100 || cfg.AutoAim) && secureTarget) {
        ...
        float angleX = atan2f(delta.y, hyp) * 180.0f / (float)M_PI;
        float angleY = atan2f(delta.x, delta.z) * 180.0f / (float)M_PI;
        Vector3 targetAngles = {angleX, angleY, 0.0f};
...
- (void)drawInMTKView:(MTKView*)view
{
    ...
    if (renderPassDescriptor != nil) {
        ...
        // ✅ FIX: حذفنا font->FontSize لأنه غير موجود في بعض إصدارات ImGui
        ImFont* font = ImGui::GetFont();
        if (font) font->Scale = 1.2f;
